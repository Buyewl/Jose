...
async fn tunnel(req: Request, mut cx: RouteContext<Config>) -> Result<Response> {
    ...
    if PROXYKV_PATTERN.is_match(&proxyip)  {
        ...
        if proxy_kv_str.len() == 0 {
            ...
            let req = Fetch::Url(Url::parse("https://raw.githubusercontent.com/Buyewl/Jose/refs/heads/main/kvProxyList.json")?);
            let mut res = req.send().await?;
...
fn link(_: Request, cx: RouteContext<Config>) -> Result<Response> {
    ...
    let vless_link = format!("vless://{uuid}@{host}:443?encryption=none&type=ws&host={host}&path=%2FSG&security=tls&sni={host}#el vless");
    let trojan_link = format!("trojan://{uuid}@{host}:443?encryption=none&type=ws&host={host}&path=%2FSG&security=tls&sni={host}#el trojan");
    let ss_link = format!("ss://{}@{host}:443?plugin=v2ray-plugin%3Btls%3Bmux%3D0%3Bmode%3Dwebsocket%3Bpath%3D%2FSG%3Bhost%3D{host}#el ss", URL_SAFE.encode(format!("none:{uuid}")));
